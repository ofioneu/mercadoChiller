...
export default function ImportacaoForm() {
  ...
  const myAwb = "9521918921"; // Substitua 'AWB_NUMBER' pelo número do AWB desejado

  const options = {
    method: "GET",
    url: "https://api-eu.dhl.com/track/shipments",
    params: { trackingNumber: "9521918921" },
    ...
    const fetchDocuments = async () => {
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          console.log(awb); // Aqui você pode fazer o que desejar com o valor do campo awb
          awbArray.push(awb);
          setAwbList(awbArray);
        });
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchData, 3600000); // Atualiza a cada 1h
            ...
            <ul>
              {awbList[0] &&
                awbList.map((awb, index) => (
                  <li key={index}>
                    <Button onClick={() => setModalAwb(true)} id="btnListAwb">
                      {awb}
                    </Button>
                  </li>
                ))}
